// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT license.

namespace Sample.TwitterSDK
{
    public class SettingsTwitter
    {
        // Constants
        public const string TwitterEndPoint = "https://api.twitter.com";

        public const string TwitterAuthEndPoint = "https://api.twitter.com";

        public const string TwitterOauthEndpoint = "oauth/access_token";

        // Configurable settings at run time

        public static string TwitterAppId = string.Empty;

        public static string TwitterAppSecret = string.Empty;

        public static string TwitterClientToken = string.Empty;

        public static string TwitterClientSecret = string.Empty;
    }
}
